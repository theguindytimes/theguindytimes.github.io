class MessagesController < ApplicationController
    ...
    def create
        ...
        respond_to do |format|
            ...
            if verify_recaptcha(:model => @message,:message => "Oh! It's an error") && @message.save
                format.html { redirect_to @message, notice: 'Message was successfully created.' }
                format.json { render :show, status: :created, location: @message }
                format.js { Feedback.feed(@message.name,@message.email,@message.subject,@message.body).deliver
                            Feedback.response("Message from #{@message.name}",@message.email,"Site response: " + @message.subject,@message.body).deliver
                }
            else
